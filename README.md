...
| user_account |格式為<local-part>@<domin>。<local-part>的限制條件只能包含英文字母Aa~Zz、數字0~9、特殊符號!#$%&'*+-/=?^_{|}~.`，關於點號不能作為開頭或結尾、不可連續出現，系統不接受” “。<domin>結構以點號進行分割，每段長度小於等於63字元，全部不超過255字元，只能包含英文字母Aa~Zz、數字0~9、特殊符號-，但特殊符號-不能當作開頭、結尾，系統不接受(test)註釋的方式。|
