...
| 發票紀錄表 |使用者可以為每一筆交易都附上一張發票，可以方便使用者兌獎|
| 存錢挑戰與目標追蹤 |使用者可以為自己設立一個存錢的目標，且系統會跟你說目前離你的目標還差多遠|    
| 每月預算表 |使用者可以為自己設定每個月什麼類別可以花多少錢| 
| 帳單紀錄表 |使用者可以新增需要處理的帳單，系統會根據要到達日期與否提醒使用者快到期了| 
| 資產紀錄表 |使用者可以新增在自己名下有哪些資產|
| 每月收支表 |使用者可以新增固定金額固定時間的扣款訊息，讓系統可以自動紀錄|
| 通知表 |根據不同事情來讓系統通知使用者|
---
## 使用案例範例
| 使用情境           | 角色介紹                 | 案例描述                                                                 |
